...
description: vectorstores is the leading framework for utilizing context engineering to build LLM applications in JavaScript and TypeScript.
---

vectorstores is a **framework for utilizing context engineering to build generative AI applications** with large language models. From rapid-prototyping RAG chatbots to deploying multi-agent workflows in production, LlamaIndex gives you everything you need — all in idiomatic TypeScript.
...
<div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 my-6">
  ...
  <a href="#use-cases" className="block rounded-lg border border-gray-600/40 p-4 hover:border-gray-400 hover:bg-gray-700/20 no-underline">
    ...
    <p className="text-sm text-gray-400 no-underline">See what you can build with vectorstores.</p>
  </a>
...
[Agents](/framework/tutorials/agents/1_setup) are LLM-powered assistants that can reason, use external tools, and take actions to accomplish tasks such as research, data extraction, and automation. 
vectorstores provides foundational building blocks for creating and orchestrating these agents.

### What are workflows?

[Workflows](/framework/tutorials/workflows) are multi-step, event-driven processes that combine agents, data connectors, and other tools to solve complex problems. 
With vectorstores you can chain together retrieval, generation, and tool-calling steps and then deploy the entire pipeline as a microservice.
...
The most popular example is [Retrieval-Augmented Generation (RAG)](/framework/getting_started/concepts), but the same idea powers agent memory, evaluation, extraction, summarisation, and more.

vectorstores gives you:
...
You can learn more about these concepts in our [concepts guide](/framework/getting_started/concepts).

## Use cases

Popular scenarios include:

- [LLM-Powered Agents](/framework/tutorials/agents/1_setup)
- [Indexing and Retrieval](/framework/tutorials/rag)
- [Extracting Structured Data](/framework/tutorials/structured_data_extraction)
- [Custom Orchestration with Workflows](/framework/tutorials/workflows)
...
<iframe
  ...
  aria-label="vectorstores Starter"
  aria-description="Interactive starter for vectorstores"
  src="https://stackblitz.com/github/run-llama/vectorstores/tree/main/examples?embed=1&file=starter.ts"
/>

Want to learn more? We have several tutorials to get you started:

- [Installation + Runtime Guide](/framework/getting_started/installation)
- [Create your first agent](/framework/tutorials/agents/1_setup)
- [Learn how to index data and chat with it](/framework/tutorials/rag)
- [Learn how to write your own workflows and agents](/framework/tutorials/workflows)
...
We 💜 contributors! View our [contributing guide](https://github.com/run-llama/vectorstores/blob/main/CONTRIBUTING.md) to get started.
