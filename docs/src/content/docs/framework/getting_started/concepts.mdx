...
In LlamaIndex, you can build agentic applications by using the workflows to orchestrate a sequence of steps and LLMs. You can [learn more about workflows](/framework/tutorials/workflows).
...
You can [learn more about agents](/framework/tutorials/basic_agent).

## Retrieval Augmented Generation (RAG)

Retrieval-Augmented Generation (RAG) is a core technique for building data-backed LLM applications with LlamaIndex. It allows LLMs to answer questions about your private data by providing it to the LLM at query time, rather than training the LLM on your data. To avoid sending **all** of your data to the LLM every time, RAG indexes your data and selectively sends only the relevant parts along with your query. You can [learn more about RAG](/framework/tutorials/rag).

## Use cases

There are endless use cases for data-backed LLM applications but they can be roughly grouped into four categories:

[**Agents**](/framework/tutorials/basic_agent):
An agent is an automated decision-maker powered by an LLM that interacts with the world via a set of [tools](/framework/modules/agents/tool). Agents can take an arbitrary number of steps to complete a given task, dynamically deciding on the best course of action rather than following pre-determined steps. This gives it additional flexibility to tackle more complex tasks.

[**Workflows**](/framework/tutorials/workflows):
...
[**Structured Data Extraction**](/framework/tutorials/structured_data_extraction):
...
[**Query Engines**](/framework/modules/rag/query_engines):
...
[**Chat Engines**](/framework/modules/rag/chat_engine):
