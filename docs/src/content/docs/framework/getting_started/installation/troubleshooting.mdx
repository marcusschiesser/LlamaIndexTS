...
description: Common issues and solutions when installing and deploying vectorstores applications.
---

This guide addresses common issues you might encounter when installing and deploying vectorstores applications across different environments.
...
- `@vectorstores/readers` - May not work in serverless environments
- `@vectorstores/huggingface` - Limited browser/edge compatibility
...
const initializeLlamaIndex = async () => {
  const { VectorStoreIndex, SimpleDirectoryReader } = await import("@vectorstores/core");
...
**Solution:** Use `@vectorstores/env` for environment compatibility:
...
- [GitHub Issues](https://github.com/run-llama/vectorstores/issues)
...
1. **Check specific deployment guides:**
   - [Server APIs](/framework/getting_started/installation/server-apis)
   - [Serverless Functions](/framework/getting_started/installation/serverless)
   - [Next.js Applications](/framework/getting_started/installation/nextjs)
