...
Agent Workflows are a powerful system that enables you to create and orchestrate one or multiple agents with tools to perform specific tasks. It's built on top of the base [`Workflow`](/framework/modules/agents/workflows) system and provides a streamlined interface for agent interactions.
...
```typescript
import { agentToolCallEvent, agentStreamEvent } from "@vectorstores/workflow";
...
```typescript
import { tool } from "@vectorstores/core";
import { multiAgent, agent } from "@vectorstores/workflow";
import { openai } from "@vectorstores/openai";
import { z } from "zod";
