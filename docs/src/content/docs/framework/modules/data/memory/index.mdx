...
In vectorstores, you can create memory by using the `createMemory` function. This function will return a `Memory` object, which you can then use to store and retrieve information.
...
import { createMemory, staticBlock } from "@vectorstores/core";
...
If you call `get`, messages are usually retrieved in the vectorstores format (type `ChatMessage`). If you specify the `type` parameter using `get`, you can return the messages in different formats. E.g.: using `type: "vercel"`, you can return the messages in Vercel format:
...
```ts
import { createMemory, factExtractionBlock, staticBlock, vectorBlock } from "@vectorstores/core";
...
```ts twoslash
import { createMemory, loadMemory } from "@vectorstores/core";
...
Want to learn more about the Memory class? Check out our example codes in [Github](https://github.com/run-llama/vectorstores/tree/main/examples/agents/memory).
