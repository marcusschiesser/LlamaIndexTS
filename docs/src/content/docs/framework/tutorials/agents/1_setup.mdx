...
In this guide we'll walk you through the process of building an Agent in JavaScript using the vectorstores library, starting from nothing and adding complexity in stages.
...
## Install vectorstores

You'll need to have a recent version of [Node.js](https://nodejs.org/en) installed. Then you can install vectorstores by running

```package-install
npm i llamaindex @vectorstores/openai @vectorstores/readers @vectorstores/huggingface @vectorstores/workflow
```
