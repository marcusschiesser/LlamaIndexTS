...
```package-install
npm i llamaindex @vectorstores/openai @vectorstores/huggingface
```

### New dependencies

We'll be bringing in `SimpleDirectoryReader`, `HuggingFaceEmbedding`, `VectorStoreIndex`, and `QueryEngineTool`, `OpenAIContextAwareAgent` from vectorstores, as well as the dependencies we previously used.

```javascript
import { QueryEngineTool, Settings, VectorStoreIndex } from "@vectorstores/core";
